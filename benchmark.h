...
public:
    Benchmark(  const std::string& data_filename,
        ...
        // Add artificial values(index) to keys.
    ...
    void Run() {
        ...
        if (!index.applicable(unique_keys_, data_filename_)) {
            std::cout << "index " << index.name() << " is not applicable" << std::endl;
            return;
        }
        ...
        if constexpr (!sosd_config::fast_mode) {
            ...
            if (perf_ || cold_cache_ || fence_) {
                util::fail(
                    "Perf, cold cache, and fence mode require full builds. "
                    "Disable fast mode.");
            }
            DoEqualityLookups<Index, false, false, false>(index);
            PrintResult(index);
    ...
    bool uses_binary_search() const {
        return (searcher_.name() == "BinarySearch") || (searcher_.name() == "BranchlessBinarySearch");
    }

    bool uses_lienar_search() const { return searcher_.name() == "LinearSearch"; }

private:
    bool CheckResults(uint64_t actual, uint64_t expected, KeyType lookup_key, SearchBound bound) {
        if (actual != expected) {
    ...
    void PrintResult(const Index& index) {
        ...
        std::cout << index.name() << " failed" << std::endl;
        }
        if (csv_) {
